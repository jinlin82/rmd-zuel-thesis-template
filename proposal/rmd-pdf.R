...
process_tangle.block <- function (x) {
    ...
    rmarkdown::render(input=paste(args[1], ".rmd", sep=""),
                      ...
                      ## 使用pdf_document()函数使得 yaml 前言无法产生作用
                                        # rmarkdown::pdf_document(latex_engine = "xelatex", keep_tex=TRUE, number_sections = T),
                      encoding="utf-8")
}else{
    rmarkdown::render(input=paste(args[1], ".md", sep=""),
	                  "bookdown::pdf_document2",
                      encoding="utf-8")}
